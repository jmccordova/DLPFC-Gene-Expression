...
dir.create(paste(exportdir, exportsubdir, sep = "/"), recursive=TRUE)
  ...
  # Step 5.4: Use logistic regression values for interpretation
    ...
    levels(trainset.binomial$diagnosis)[match("1",levels(trainset.binomial$diagnosis))] <- "NCTL"
    levels(trainset.binomial$diagnosis)[match("9",levels(trainset.binomial$diagnosis))] <- "CTL"
    # Step 5.4.2: Get the model
    fit <- vglm(diagnosis ~ ., family = binomial(), data = trainset.binomial) # vglm = vector GLM
